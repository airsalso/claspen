...
import { AGENT_VALIDATORS } from './src/constants.js';
...
async function main(webUrl, repoPath, configPath = null, pipelineTestingMode = false, disableLoader = false) {
  ...
  try {
    ...
    global.__SHANNON_TARGET_DIR = sourceDir;
  ...
  if (startPhase <= 1) {
    ...
    if (await AGENT_VALIDATORS['pre-recon'](sourceDir)) {
      await updateSessionProgress('pre-recon');
    } else {
      console.log(chalk.red('❌ Pre-reconnaissance validation failed: Deliverable missing.'));
    }
  }
  ...
  if (startPhase <= 2) {
    ...
    // Validate deliverables
    if (await AGENT_VALIDATORS['recon'](sourceDir)) {
      await updateSessionProgress('recon');
    } else {
      console.log(chalk.red('❌ Reconnaissance validation failed: Deliverable missing.'));
    }
  }
