...
export const getRetryDelay = (error, attempt) => {
  ...
  const baseDelayVal = parseInt(process.env.SHANNON_RETRY_BASE_DELAY_MS) || 1000;
  const maxDelayVal = parseInt(process.env.SHANNON_RETRY_MAX_DELAY_MS) || 30000;

  const baseDelay = Math.pow(2, attempt) * baseDelayVal;
  const jitter = Math.random() * baseDelayVal;
  return Math.min(baseDelay + jitter, maxDelayVal);
};
