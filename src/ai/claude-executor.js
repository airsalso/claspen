...
// Helper to get the correct LLM provider
function getLLMProvider(config = {}) {
  // Always use VLLM provider as requested
...
async function validateAgentOutput(result, agentName, sourceDir) {
  ...
  try {
    // Check if agent completed successfully (be lenient with empty text result)
    if (!result.success) {
      console.log(chalk.red(`    ❌ Validation failed: Agent execution reported failure`));
      return false;
    }
...
async function runClaudePrompt(prompt, sourceDir, allowedTools = 'Read', context = '', description = 'Claude analysis', agentName = null, colorFn = chalk.cyan, sessionMetadata = null, auditSession = null, attemptNumber = 1) {
  ...
  try {
    ...
    const options = {
      ...
      maxTurns: parseInt(process.env.SHANNON_AGENT_MAX_TURNS) || 100, // Reduced to prevent infinite loops while allowing enough depth
      ...
      mcpServers,
      agentName, // Pass agent name for context-aware tool behavior
    };
    ...
    const HEARTBEAT_INTERVAL = parseInt(process.env.SHANNON_HEARTBEAT_INTERVAL_MS) || 30000; // 30 seconds

    const provider = getLLMProvider();

    try {
      for await (const message of provider.query({ prompt: fullPrompt, options })) {
        messageCount++;
