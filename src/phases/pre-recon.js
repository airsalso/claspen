...
import { DeliverableType, DELIVERABLE_FILENAMES } from '../../mcp-server/src/types/deliverables.js';
...
async function runTerminalScan(tool, target, sourceDir = null) {
  ...
  try {
    ...
    switch (tool) {
      ...
      case 'whatweb':
        console.log(chalk.blue(`    🔍 Running ${tool} scan...`));
        const whatwebOpenTimeout = process.env.SHANNON_WHATWEB_OPEN_TIMEOUT || 30;
        const whatwebReadTimeout = process.env.SHANNON_WHATWEB_READ_TIMEOUT || 60;
        console.log(chalk.gray(`    Command: whatweb --open-timeout ${whatwebOpenTimeout} --read-timeout ${whatwebReadTimeout} ${target}`));
        result = await $({ silent: true, stdio: ['ignore', 'pipe', 'ignore'] })`whatweb --open-timeout ${whatwebOpenTimeout} --read-timeout ${whatwebReadTimeout} ${target}`;
        ...
        if (await fs.pathExists(schemasDir)) {
          ...
          if (apiSchemas.length > 0) {
            ...
            for (const schemaFile of apiSchemas) {
              ...
              try {
                const schemathesisMaxFailures = process.env.SHANNON_SCHEMATHESIS_MAX_FAILURES || 5;
                result = await $({ silent: true, stdio: ['ignore', 'pipe', 'ignore'] })`schemathesis run ${schemaPath} -u ${target} --max-failures=${schemathesisMaxFailures}`;
...
async function runPreReconWave1(webUrl, sourceDir, variables, config, pipelineTestingMode = false, sessionId = null) {
  ...
  const nmap = pipelineTestingMode ? { tool: 'nmap', output: 'Skipped', status: 'skipped' } : results[0];
  const subfinder = pipelineTestingMode ? { tool: 'subfinder', output: 'Skipped', status: 'skipped' } : results[1];
  const whatweb = pipelineTestingMode ? { tool: 'whatweb', output: 'Skipped', status: 'skipped' } : results[2];
  const codeAnalysis = pipelineTestingMode ? results[0] : results[3];
  const naabu = { tool: 'naabu', output: 'Skipped', status: 'skipped', duration: 0 };
...
Report generated at: ${new Date().toISOString()}
  ...
  try {
    const filename = DELIVERABLE_FILENAMES[DeliverableType.PRE_RECON];
    const deliverablePath = saveDeliverableFile(filename, report);
  } catch (error) {
